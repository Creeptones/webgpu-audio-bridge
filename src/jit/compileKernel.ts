/**
 * compileKernel â€” the JIT pipeline entry (Apollo Frontier 5, Stage 1a).
 *
 *   source (naive scalar JS) + KernelSignature
 *     â†’ parse (acorn)            [E_PARSE on bad JS]
 *     â†’ lower/validate â†’ IR      [rejected-source on any out-of-subset construct]
 *     â†’ vectorize â†’ plan         [unsupported if not v1-emittable, e.g. stride-2]
 *     â†’ emit scalar + SIMD WAT
 *     â†’ equivalence gate         [rejected-gate if SIMD â‰  scalar on the corpus]
 *     â†’ accepted { wasm, â€¦ }
 *
 * The result is a DISCRIMINATED UNION â€” `compileKernel` NEVER throws on a user
 ...
 *
 * `@experimental` â€” exported from `webgpu-audio-bridge/experimental`, NOT the
...
export interface CompileKernelOptions {
  /** Injected WATâ†’bytes compiler (wabt in tests/build; a binary encoder in the
   *  live browser worker). Required â€” the gate cannot run without it. */
  ...
  /** Enable relaxed SIMD fused math for speed.
   *  Default false: bit-exact emit of `mul` + `add`.
   *  When true, the gate uses SIMD tolerance instead of strict/ULP checks. */
   ...
   *  independent voices per v128) instead of the scalar fallback â€” gate-proven lane
   *  `j` â‰¡ a scalar run of voice `j`. Default 1 (the single-voice scalar path,
 ...
 *  The width is NOT here â€” it is carried by the IR (`ir.width` / `ir.signature`),
...
export interface CompileIrOptions {
   ...
   *  LOWERING â€” scalar WASM â‰  source). N/A on the IR/token path: there the IR IS
   *  the spec and SIMDâ‰¡scalar is the safety, so leave it undefined. Threaded only
   *  from the legacy `compileKernel(source)` entry. */
  readonly jsSource?: string;
  /** Polyphonic voice batch (Apollo Frontier 7, Stage 4) â€” see `CompileKernelOptions.voices`. */
  readonly voices?: number;
}

/** Options for the token entry (`compileTokens`). Identical to `CompileIrOptions`
 *  minus `jsSource` (the token path has no separate JS author â€” the IR is the
 *  spec). */
...
export type CompileResult =
  | {
       ...
       *  allocate + seed the per-generation state slab â€” they cannot be derived
       ...
       *  multiple of `plan.laneWidth`) for the voice-SIMD path â€” the runtime sizes its
...
export function compileKernel(
  ...
  // parse + lower/validate â€” rejection is a value, not an exception.
  let ir: IrKernel;
  try {
...
/**
 * The IR back-half of the pipeline (Apollo Frontier 6, Stage 1): vectorize â†’ emit
 * scalar + SIMD WAT â†’ equivalence gate â†’ accepted SIMD bytes. Shared by the legacy
 * `compileKernel(source)` front-half (parse â†’ lower) and the token front-half
 * (`compileTokens` / the kernel grammar). Behavior is IDENTICAL to the pre-Stage-1
 * inline block â€” `compileKernel` is now `parse â†’ lower â†’ compileIr`.
 *
 * The width + signature ride in `ir` (resolved by lowering or token validation), so
 * there is no width override here. `opts.jsSource` is the gate's THIRD oracle â€” pass
 * it ONLY from `compileKernel(source)`; on the IR/token path the IR is the spec, so
 * leave it undefined (SIMDâ‰¡scalar is the safety).
 */
export function compileIr(ir: IrKernel, opts: CompileIrOptions): CompileResult {
  const exportName = opts.exportName ?? "kernel";
  const voices = opts.voices ?? 1;
  const fastMath = opts.fastMath ?? false;

  // vectorize â€” surfaces v1-non-emittable shapes as `unsupported` (â†’ JS fallback);
  // picks the lowering mode: stateless â‡’ "simd-time"; stateful + a power-of-W voice
  // batch â‡’ "simd-voice" (Frontier 7, Stage 4); else "scalar" (the recurrence wall).
  ...
  // run never re-reads a wrapped slot â€” a ring-addressing / off-by-one bug would hide).
  const maxBuf = Math.max(0, ...(ir.stateBuffers ?? []).map((b) => b.length));
  const longRun = maxBuf > 0 ? [256, 512, maxBuf * 2 + 17] : [256, 512];

  // â”€â”€ Frontier 7, Stage 4: the voice-axis SIMD path â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€
  // A stateful kernel compiled across W independent voices (lane j = voice j). The
  // deliverable is the voice module; the gate proves lane j â‰¡ evalReference(voice j),
  ...
  if (vec.plan.mode === "simd-voice") {
    ...
    const voiceWat = emitVoiceSimdModule(ir, W, exportName, fastMath);
    const nValues = [...CORPUS_N_VALUES, ...longRun];
    const baseSeed = opts.corpus?.seed ?? 0xc0ffee;
    // W per-lane corpora â€” same n-values (case indices align) but DISTINCT values per
    ...
    const gate = runGate({
      ...
      compileWat: opts.compileWat, maxUlpF32: opts.maxUlpF32, voiceMode: true, fastMath,
    });
  ...
  // â”€â”€ Frontier 7: the scalar-only (stateful, single-voice) path â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€
  // No SIMD candidate (the recurrence is not time-axis vectorizable), so the
  // deliverable IS the scalar module and the gate proves scalar WASM â‰¡ evalReference
  ...
  if (vec.plan.scalarOnly) {
    ...
    const gate = runGate({ ir, scalarWat, simdWat: scalarWat, corpus, compileWat: opts.compileWat, maxUlpF32: opts.maxUlpF32, scalarOnly: true, fastMath: false });
    if (gate.status === "unsupported") return { status: "unsupported", reason: gate.reason ?? "unsupported", gate };
    if (gate.status === "rejected-gate") return { status: "rejected-gate", gate };
  ...
  const simdWat = emitSimdModule(ir, exportName, fastMath);

  const corpus = buildCorpus(ir.signature, opts.corpus);
  const gate = runGate({ ir, scalarWat, simdWat, corpus, compileWat: opts.compileWat, jsSource: opts.jsSource, maxUlpF32: opts.maxUlpF32, fastMath });

  if (gate.status === "unsupported") return { status: "unsupported", reason: gate.reason ?? "unsupported", gate };
  if (gate.status === "rejected-gate") return { status: "rejected-gate", gate };

  // accepted â€” produce the deliverable SIMD bytes.
...
/**
 * The TOKEN entry (Apollo Frontier 6, Stage 1): a kernel grammar token stream â†’
 ...
 * â€” the bridge from the grammar's `{ error, at? }` shape (a token index, no source
 ...
 *
 * No `jsSource`: the token stream IS the spec, so the gate's SIMDâ‰¡scalar check is
