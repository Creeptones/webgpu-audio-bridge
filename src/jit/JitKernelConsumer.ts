...
export class JitKernelConsumer {
   ...
   */
  installCompiledKernel(module: WebAssembly.Module): boolean {
   ...
   */
  installCompiledKernelFromBytes(bytes: Uint8Array): boolean {
    if (!this.jitEnabled) return false;
    let inst: WebAssembly.Instance;
    try {
  ...
  private armInstance(inst: WebAssembly.Instance): boolean {
  ...
  process(
    ...
    if (q.phase === "fading") {
      ...
      if (!this.outputsFiniteB(n)) {
        // Non-finite incoming output → abort fade, snap to the JS fallback.
