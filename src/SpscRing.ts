...
export class SpscRing<S extends Schema<FieldsObject, any>> {
  ...
  /**
   * Dev-only bench shim. Mirrors `pull` exactly but uses the **proposed
   * 0.7.0 wait-flag protocol** for the trailing notify:
   *
   *   1. Run the full pull body, same as `_pullNoNotify`.
   *   2. Atomics.load lane 4 (reserved-for-0.7.0, currently always zero).
   *   3. If the loaded value is non-zero (producer-parked flag set),
   *      fire `Atomics.notify(READ_IDX_LANE, 1)`. Otherwise skip — the
   *      producer is not parked, so the notify would be a no-op anyway
   *      (engines short-circuit empty wait-lists per Investigation 1).
   *
   * NOT part of the public API. Bridge does NOT delegate to it. The shim
   * exists so `bench/Bridge.bench.ts` (Investigation 3) can measure
   * whether the lane-4 check overhead (one Atomics.load + one branch)
   * is cheaper than the per-pull notify cost it eliminates. That
   * comparison is the deciding number for the 0.7.0 wait-flag protocol
   * proposal.
   *
   * The proposed protocol's actual producer-side bookkeeping (set the
   * flag before parking, clear after wake) is NOT simulated here — that
   * cost is per-park-cycle, not per-pull, and rare in the audio use case.
   * This shim measures only the consumer-side per-pull overhead.
   *
   * Added 0.6.12-WIP. See CHANGELOG when the investigation ships.
   */
  _pullWithWaitFlag(out: FrameFor<S>): SpscPullResult {
    const r = this.pullResult;
    const readIdx = this.indices[READ_IDX_LANE]!;
    const writeIdx = Atomics.load(this.indices, WRITE_IDX_LANE); // acquire
    if (writeIdx === readIdx) {
    ...
    const invariantStored = this.invariantView !== null
      ...
      : 0;
    Atomics.store(this.indices, READ_IDX_LANE, (readIdx + 1) | 0); // release
    // The proposed wait-flag protocol: notify only if the producer-park
    // flag (lane 4) is set. In steady state with no producer parked,
    // lane 4 reads zero and the notify is skipped entirely.
    if ((Atomics.load(this.indices, 4) | 0) !== 0) {
      Atomics.notify(this.indices, READ_IDX_LANE, 1);
    }
    this._updateFlowScale(writeIdx, readIdx);
    r.ok = true;
