...
- **Stage 1b (SHIPPED @ 0.9.942)** — `MpmcWorkQueue` depth-driven `flow_scale` lane (reserved lane 7) + `flowScaleHint()`. Patch. Occupancy is the UNDELIVERED depth `signedDiff(enqueueTicket, committedFrontier)`, **not** `available()`'s claimable gap. **The one structural difference from Stage 1a — the multi-writer-soft-hint property:** `MpmcWorkQueue` is MP→**M**C, so there is no single consumer to own the controller. Each of the M competing consumers owns its OWN `AdaptiveFlowController` instance (its own heap-side PI integral) and ticks it on its successful `pull`, last-writer-wins on lane 7. This is sound for a **soft, self-correcting** hint precisely because every consumer observes the SAME global occupancy `(W − F)`: their independent integrators converge to the same neighborhood, and a stale/slightly-divergent lane read is harmless by construction (Q3 / §5 — the producer never blocks on the hint, so a transiently-wrong value only mis-paces one quantum and self-corrects). The rejected alternative (a producer-side tick in `advanceFrontier`) has the same N-instance multi-integral shape AND runs on the lazy bounded scan, off-cadence with delivery → noisier. Tests: `tests/MpmcWorkQueue.test.ts` pin 10 drives the real `pull` path deterministically (mirrors `MpmcRing` pin 9); `tests/MpmcWorkQueue.concurrent.test.ts` has each consumer worker run a byte-faithful controller and pins the M multi-writers keep the lane finite + in-range + off the seeded default under real cross-thread contention. No interleaving-fuzzer change (side channel, no protocol interaction).
- **Stage 1c (SHIPPED @ 0.9.943)** — `SpmcRing` per-consumer `flowScale[c]` lane + producer min-reduce. **The genuinely-new operation of the arc.** A 4th per-consumer lane (`PER_CONSUMER_LANES` 3→4) — unlike 1a/1b's reserved-header-lane reuse, this **grows the per-consumer region**, so it IS a SAB-layout change for this ring (still patch — experimental). **The M-consumer story is the OPPOSITE of 1b's:** `SpmcRing` already gives each consumer its own lane region, so each consumer is the **sole writer** of its `flowScale[c]` cell (no last-writer-wins race — each runs its own controller over its OWN backlog `signedDiff(writeTicket, dequeuePos[c])`, capped at capacity by the overload net). The new piece is the **producer-side MIN-reduce** in `flowScaleHint()`: an O(consumerCount ≤ 64) relaxed-load `min` over the N cells (bounded, no retry → wait-free). `min` is the correct compose because the single producer broadcasts to ALL consumers — one overfull consumer pulls the pacing down regardless of the others, so the producer paces to the SLOWEST leg (probe C: producer settles to `min(2.0, 0.125) = 0.125`). Advisory only — `push()` laps freely and never reads the lane (§5-safe). Tests: `tests/SpmcRing.test.ts` pin 11 (deterministic — per-consumer isolation, sustained-full→cell<0.5, white-box min order-independent + bounded, sustained-low→cell→2.0); `tests/SpmcRing.concurrent.test.ts` + `tests/connectFanOut.concurrent.test.ts` (each consumer worker runs a byte-faithful controller, the producer min-reduce pinned finite + in-range cross-thread). The per-consumer stride was bumped `*3`→`*4` across all three SpmcRing-touching concurrent tests + the layout pin (the SAB-size assertions are the structural gate). **No interleaving-fuzzer change** — the lane is a side channel with no protocol interaction and the min-reduce is a read-only reduction (its order-independence + bounds are proven deterministically by pin 11), matching the Stage-1a/1b decision.
