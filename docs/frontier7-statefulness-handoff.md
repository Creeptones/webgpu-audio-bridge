...
> **SHIPPED postscript (added after Stage 0 + Stage 1 landed).**
> - **Stage 0 — design** (docs commit `docs(frontier7): Stage 0`): `docs/frontier7-statefulness-semantics.md` (the recurrence theorem; the six §5 open questions settled) + `bench/state-probe.mjs`. **The z⁻¹ ordering was pinned SIMULTANEOUS (state-space), NOT the sequential this handoff recommended** — see the semantics note §2.2 for the reasoning (two-list IR order-independence + provable evalReference/WASM/JS-fallback equality). The JS-authoring `lower.ts` relaxations (§5.2) were **deferred** to a follow-up; Stage 1 ships the token/IR path.
> - **Stage 1 — the compiler** (**@ 0.9.929**): IR (`readState` + optional `stateDecls`/`stateStores`, `isStateful`, `kernelKey` state fold), grammar (`state`/`readState`/`writeState` in the one `stepGrammar` machine + masks + codec), `vectorize`→`scalarOnly`, `emitScalarModule` state threading + `paramLayout` `$__state` arg, `gate.ts` scalar-WASM≡`evalReference` mode (long corpus run), `acousticGate` stability (4096 probe + growth-ratio), `emitJsKernel` simultaneous-state fallback. Proven by `tests/stateKernel.test.ts` (one-pole + biquad gate-verified, unstable rejected, stateless SIMD unchanged + `kernelHash(gain)` pin, three-evaluator agreement). The stateless SIMD path is byte-identical (frontier-gated).
> - **NEXT: Stage 2** — the `JitKernelConsumer` persistent per-generation state slab across `process()` quanta + the independent-state crossfade (§3.6). The JS fallback already takes a trailing persistent state-slab typed-array param (the convention Stage 2 adopts). After that: Stage 3 (delay lines), Stage 4 (SIMD-across-voices), Stage 5 (surface + demo), and the deferred `lower.ts` JS-authoring path.
