...
- **Stage 2 — bench + characterize** (`bench/mpmc.bench.ts`): enqueue/dequeue latency vs producer count, contention curve, drop-rate at the envelope edge; confirm the consumer path stays inside the 10 µs budget and SPSC bench is unchanged (separate path → must be noise).
