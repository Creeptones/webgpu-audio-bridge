// worker.js — the GPU residual producer (TIER 3 of the four-tier demo).
//
// Byte-for-byte the same residual kernel as
// examples/hybrid-residual-stereo/worker.js: an interleaved stereo residual of
// harmonic partials (k = 2..N_PARTIALS+1, 1/k roll-off, per-partial slow LFO),
// rendered TWICE with a small per-channel detune + phase offset so the
// upper-harmonic layer has an intrinsic stereo width. The fundamental is NOT
// here — that's the CPU carrier (tier 2) in the worklet.
//
// THE TWO NEW PIECES the four-tier stack adds live OUTSIDE this file:
//   - Tier 4's azimuth macro drives an equal-power L/R PAN of the residual in
//     the WORKLET (so it tracks the gesture predictively without a GPU
//     round-trip); the residual rendered here stays the intrinsic-width source.
//   - The convolution / room tail (handoff §3.4) is the documented follow-on —
//     it needs a two-pass persistent-history WGSL dispatch and is deferred to a
//     later patch so the four-tier ship stays composition, not new GPU DSP.
//
// This worker's ONLY four-tier-specific responsibility is the one it already
// fulfilled in the stereo demo: it measures `lastReadbackUs` and posts it in
// its telemetry. main.js relays that number to the worklet, which feeds it to
// the macro bridge's recordReadbackLatency() so tier 4's predictive lead tracks
// the live GPU-readback wall (handoff §3.3). No new measurement code.
//
// Stall simulation identical to the mono/stereo demos.

import { Bridge, BridgeBlockProducer } from "../../dist/index.js";
import { makeBlockSchema } from "./schema.js";
