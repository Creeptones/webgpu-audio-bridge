...
**Standard mode** (reserved — `MessageChannelBridge<S>`) — same schema DSL surface, **MessageChannel + transferable ArrayBuffer** transport, target latency floor 5–50 ms vs Turbo's sub-µs. Right for: prototyping before you've configured COOP/COEP, control-plane updates in unisolated embeds, telemetry channels, non-audio-critical paths. **Not for audio rate.** A full design note covering API shape, versioning slot, MVP scope, and implementation cost lives at [`docs/standard-mode-design.md`](./docs/standard-mode-design.md); the [`ROADMAP.md`](./ROADMAP.md) reserved-slot section summarises the decision-pending status. Timing is open. See [Browser support matrix](#browser-support-matrix) for what works in each tier once Standard ships.
...
**Interpolation modes — `'taylor' | 'hermite'`, closed at 1.0 (0.8.10).** The `interpolationMode` field on `f{32,64}TrajectoryArray` selects how the consumer reconstructs the signal: `'taylor'` (default, single-frame extrapolation of the producer-stamped derivatives) or `'hermite'` (C¹-continuous cubic interpolation between two consecutive frames, requires `order >= 2`; entry point is the two-frame `Bridge.evaluateHermiteInto(prev, curr, t, segmentSec, out)`). The union is **closed at 1.0** — a future quintic-Hermite path that consumes acceleration at both endpoints for full C² continuity is **deferred to 1.x** as a separate `'quintic-hermite'` value, landing via an additive minor bump (e.g. 1.1.0) rather than an in-place widening of the 1.0 union. The additive-name shape lets 1.0 consumer `switch` statements stay exhaustive without a default branch; a 1.x consumer that fails to handle the new arm sees a compile error rather than silent fall-through.
