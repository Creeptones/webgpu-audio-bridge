...
- ✅ **0.6.8 — Internal `SpscRing` extract**. The SAB / Atomics core of `Bridge<S>` lifts into a new internal class `SpscRing<S>` (`src/SpscRing.ts`). No public-API change, no wire-format change, no exported symbol additions — every `Bridge<S>` method continues to work bit-identically and the 1 M-frame concurrent SPSC stress passes the seam unchanged. Preparatory for the 0.6.10 composable exports; 0.6.9 will split `FrameSmoother` / `ConsumerClockRecovery` / `AdaptiveFlowController` out along the same seam.
