...
- **Version**: 0.9.944 (June 2026). Active 0.9.x soak cohort heading toward 1.0; see [`ROADMAP.md`](./ROADMAP.md#the-10-trigger). Pre-1.0 is **deliberate policy**, not abandonment — 1.0 means a settled-API stability commitment, not a feature checkpoint, and the [`CLAUDE.md`](./CLAUDE.md) versioning policy treats each 0.9.x patch as a maturity checkpoint rather than a race-to-1.0 stepping stone.
...
- **WebGPU support is broad but still deployment-sensitive.** Chrome / Edge, current Firefox channels on supported desktop platforms, and current Safari releases expose WebGPU in many production configurations, but support is still not universal across every OS/GPU/browser combination and should not be treated as a guaranteed Baseline capability at deploy time. The library's Turbo-mode core only needs `SharedArrayBuffer` + `Atomics` + `AudioWorklet` and works wherever those are available; **the `BridgeGPUSource` helper specifically** is gated on WebGPU availability and so inherits the WebGPU rollout above.
...
**See it (and hear the loop close) — `examples/god-node-hotswap/` (0.9.900).** [`examples/god-node-hotswap/`](./examples/god-node-hotswap/) (`npm run dev:god-node-hotswap`, http://localhost:5183) composes all of the above end-to-end on the audio thread. Two **different-schema** patches stream side by side — A = `{ freq(traj), amp }`, B = `{ freq(traj), amp, res, detune }` — and "Morph to B" performs a click-free cross-schema swap: `HotSwapConsumer` reconstructs both per quantum, the C² (quintic) weight drives a per-sample equal-power blend, and `migratePlan(A, B)` is *executed* (the new `res` / `detune` fields ramp in from their plan default across the same window, so B's colour eases in instead of popping). The morph also closes the **self-rewriting loop**: at button-press the page emits patch B's *entire* AudioWorklet module from its schema (`emitWorkletProcessorModule`), Blobs it (`toWorkletModuleURL`), and `addModule`s it **live** into the running `AudioContext` — a brand-new read path materialized into the audio realm while sound plays — then runs it decoding B's live ring in real time. That's the bridge rewriting its own consumer path mid-stream, LLM-free. (Runtime emit takes the documented CSP trade-off: `addModule(blob:)` needs `blob:` in `script-src`/`worker-src`; strict-CSP apps use the build-step path — write the emitted module to a `.js` file your bundler serves. See [Getting the reader into the worklet](#getting-the-reader-into-the-worklet-0947).)
...
**Why the model order is chosen by field order** (the probe's key finding): a constant-acceleration filter fed only positions estimates acceleration from the *second difference* of noisy positions, which amplifies noise and ends up **worse than a hold** — so position-only fields use `"cv"`, where the first-difference velocity is robust. When stamped derivatives exist, `"ca"` captures curvature and wins the noisy and stalled regimes. A cold or under-observed filter reports an enormous variance, so `pullKalmanPredictedLatest` folds it straight into the same confidence→horizon crossfade `pullPredictedLatest` already uses (`w = c_horizon · clamp01(1 − maxVariance/varianceFloor)`, with an optional `confidenceFloor` cliff). Allocation-free after construction; heap-only (never touches the SAB). Same Apollo discipline as the Hermite work: closed-form, left-to-right f64 accumulation, bit-exact-reasoned for the WASM ports. The **WASM scalar kernels** ship (0.9.903 — `kalmanIngestCvF64` / `kalmanPredictCvF64` + the CA pair on `WorkletConsumer`), bit-exact to the JS reference, and the **f64x2 SoA SIMD kernels** (0.9.904 — `…SoaSimd` variants) complete the arc, bit-exact to scalar and **1.5–1.6× faster** (`npm run bench:kalman-simd`). The SIMD path uses a struct-of-arrays state layout (each derivative/covariance element contiguous across lanes) so the vectorized loads stay gather-free. By default the pull predicts at a **fixed** lead off the freshest frame's stamp, which freezes through a famine (constant `dt`, constant covariance, constant weight). Pass `famineAwareHorizon: true` (with the real advancing `consumerNs` each quantum, PLL locked) to predict instead at the PLL-mapped **consumer** time + lead — an *advancing* target: through a producer stall the consumer clock keeps moving while the producer stamp is frozen, so the horizon and covariance **grow**, fading the weight monotonically to `0` ⇒ an exact hold as the stall lengthens (the predictor's headline benefit; `0.9.905`). The result's `forwardDistanceSeconds` reports the true horizon the covariance grew over.
...
The **kernel grammar** (Apollo Frontier 6, `0.9.918`–`0.9.921`) extends this: a DSP kernel can be expressed as a closed-grammar **token stream** instead of JS, content-addressed and gate-verified through the same equivalence gate, with a `KernelCache` that makes a repeated kernel free. A token stream passes a **three-gate stack** on its way to audio — **syntax** (well-formed IR), **equivalence** (SIMD ≡ scalar, bit-exact), and **acoustic** (gate #3, `0.9.921`): the cache runs the accepted kernel over a fixed deterministic probe and ACCEPTs iff the result is finite + within sane bounds (no NaN/Inf, no runaway peak/DC), attaching an amplitude-invariant spectral **fingerprint** to every characterized kernel (the basis for "sounds-like" search + a future model's feature vector). Gate #3 is *acoustic sanity + a fingerprint, not taste* — and needs no wasm (gate #2 already proved SIMD ≡ the IR reference, so it profiles a pure JS evaluation of the IR). Live demo: `npm run dev:kernel-palette` (`examples/kernel-palette/`, port 5186 — pick a kernel from a palette; it compiles + gate-verifies and live-swaps into the running AudioWorklet, re-picking it is a visible **cache hit**, and the HUD shows the acoustic fingerprint). Design note: [`docs/frontier6-grammar-design.md`](./docs/frontier6-grammar-design.md).

**Stateful kernels + SIMD across voices** (Apollo Frontier 7, `0.9.929`–`0.9.932`) lift the JIT's stateless ceiling: a kernel can carry single-sample **state registers** (`z⁻¹`, biquads, IIR feedback — Stage 1/2) and fixed-integer **delay-line ring buffers** (`z⁻N`: echo, comb filters, Karplus-Strong — Stage 3), running click-free across `process()` quanta with each generation's recurrence in its own lane-packed slab. Because a loop-carried recurrence is structurally un-vectorizable along *time* (the recurrence wall), these run **scalar** — until polyphony hands SIMD back along a different axis. **Stage 4 (`0.9.932`) re-engages SIMD across VOICES**: `V` independent voices each run the SAME stateful kernel with their OWN state + per-voice scalars, so packing `W` voices per `v128` (lane = voice) advances all `W` recurrences lane-parallel in one time loop — **bit-exact** per lane (no cross-lane op, so the gate is stronger than the time-axis ULP budget). The runtime is a `voices` knob threaded the whole way (`connectJit({ tokens, signature, voices })` → the compile request → the voice-equivalence gate → the worklet install guard): caller I/O is **voice-interleaved** (`x[i·V + v]`), and `process()`'s `scalars` are **per-voice** (an array, length `V`) or **broadcast** (a number). `voices === 1` and stateless kernels stay byte-for-byte identical (the frontier gate). The measured speedup approaches the lane count — `bench:jit` Cell 5: **3.94× (f32, W=4) / 2.02× (f64, W=2)**. Live demo: `npm run dev:poly-synth` (`examples/poly-synth/`, port 5188 — a playable 8-voice synth; each key gates one voice of a per-voice one-pole + feedback-comb kernel, and the gate-verified voice-SIMD fades in over the JS fallback click-free).
...
- **Small runtime dependency footprint.** The stable bridge core is dependency-free, while the published package currently carries `acorn` for the opt-in experimental JIT parser. Everything else is in `devDependencies` for the build/test toolchain. A fork is `git clone` + `npm install` + you're running the same code we ship.
